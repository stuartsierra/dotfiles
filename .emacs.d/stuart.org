...
** Default packages
   ...
   depends on clojure-mode 5.9 / emacs 25 / pkg-info 0.4 / queue 0.2 /
   seq 2.16 / sesman 0.3.2 / spinner 1.7
   ...
   #+BEGIN_SRC emacs-lisp :results silent
     (defvar stuart/stable-packages
       '(
         ...
         ruby-mode
         s
         sesman  ; CIDER dependency
         smex
         ))
   #+END_SRC

   Some packages are not available in MELPA-stable:
   ...
   #+BEGIN_SRC emacs-lisp :results silent
     (defvar stuart/unstable-packages
       '(
         ...
         htmlize
         linum-off
         queue   ; CIDER dependency
         restclient
         seq  ; CIDER dependency
         spinner  ; CIDER dependency
         xml-rpc
         ))
   #+END_SRC
...
** Org-babel and Clojure (CIDER)
   ...
   #+BEGIN_SRC emacs-lisp :results silent
     ...
     (defun org-babel-execute:clojure (body params)
       ...
       (when (not (cider-current-repl 'clj))
...
*** CIDER session management current

Since (some version) CIDER manages multiple REPL sessions with the
[[https://github.com/vspinu/sesman][Sesman]] library. As a result, a buffer must be linked to a session
before you can evaluate Clojure in it. See [[https://github.com/clojure-emacs/cider/issues/2464][CIDER #2464]].

Since [[https://github.com/clojure-emacs/cider/issues/2483][CIDER #2483]] sessions are automatically linked for dependency
files found through =M-.=, but this doesn't work for an arbitrary file
at an arbitrary location.

This is a workaround [[https://github.com/clojure-emacs/cider/issues/2464#issuecomment-425671148][suggested in #2464]]:

#+BEGIN_SRC emacs-lisp :results silent
  (defun stuart/sesman-link-last-session ()
    (interactive)
